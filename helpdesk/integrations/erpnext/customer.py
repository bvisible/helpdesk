...
# //// Neoffice ▼▼▼ — the same behaviour as CustomCustomer, as doc_events.
# //// The class above is no longer registered in override_doctype_class (see
# //// hooks.py): neoffice_theme overrides the Customer class too, and frappe keeps
# //// one override per doctype, the last app installed winning in silence. Doc
# //// events never erase one another, so the integration keeps working next to the
# //// theme's class. Upstream made the same move later (main 1.30:
# //// helpdesk.integrations.erpnext.customer.after_insert & co. in doc_events):
# //// at that merge, take upstream's functions and drop these.
...
def before_rename(doc, method, olddn, newdn, merge=False):
    validate_rename_conflict("Customer", olddn, newdn, merge)


def after_rename(doc, method, olddn, newdn, merge=False):
    cascade_rename("Customer", olddn, newdn, merge)
